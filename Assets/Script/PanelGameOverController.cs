...
public class PanelGameOverController : MonoBehaviour
{
    ...
    // パネル表示
    public void DispPanel(List<BaseWeaponSpawner> weaponSpawners, bool clearFlag)
    {
        // クリアならタイトル書き換え
        if (clearFlag)
        {
            Text textResultTitle = transform.Find("TextResultTitle").GetComponent<Text>();
            textResultTitle.text = "CLEAR!!";
        }
